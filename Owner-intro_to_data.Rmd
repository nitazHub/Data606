...
mean(cdc$weight)
sd(cdc$weight)
ldev <- zscore(cdc$weight) <= 1
table(ldev)
barplot(table(ldev))
table(ldev)/20000
ldev2 <- cdc$weight - (mean(cdc$weight)+sd(cdc$weight)) < 0
table(ldev2)
```
    *The above steps:
    1.calculated mean & std eviation of the weight variable
    2.created a logical dataframe (ldev) to indicate TRUE for every row that is within 1 std deviation of the mean, using the zscore function
    3.used the table function to display the contents of the ldev dataframe
    4.used the barplot function to graphically display the contents of the ldev dataframe
    5.divided the ldev dataframe by the number of rows to get the proportions of the number of respondents within 1 std dev of the mean (85%)
    *check my work* 
    ***
    6.created a second logical dataframe (ldev2) to indicate TRUE for every row that is whose weight is less than 0 when subtractiing the sum of the mean and the std dev.  This should equal the number of people within 1 std dev of the mean using zscore
    7.divided the ldev2 dataframe by the number of rows to get the proportions of the number of respondents with a value of TRUE.(85%)
