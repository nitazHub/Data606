...
```{r num-sum-height-weight, eval=TRUE}
summary(cdc$height)
summary(cdc$weight)
```
    
    Compute the relative frequency distribution for
    `gender` and `exerany`. 
```{r rel-freq-dist-gen-exer, eval=TRUE}
table(cdc$gender)/20000
table(cdc$exerany)/20000
```
    
    How many males are in the sample? 
```{r num-males, eval=TRUE}
table(cdc$gender)
```
What proportion of
    the sample reports being in excellent health
```{r prop-ex-health, eval=TRUE}
...
_Men are split around 50/50 but women mostly have not smoked 100 cigarettes_
...
to see the sixth variable of the 567^th^ respondent, use the format

```{r cdc-row567-column6, eval=TRUE}
cdc[567,6]
```

which means we want the element of our data set that is in the 567^th^
row (meaning the 567<sup>th</sup> person or observation) and the 6<sup>th</sup> column (in this case, weight). We know that `weight` is the 6<sup>th</sup> variable because it is the 6<sup>th</sup> entry in the list of variable names
...
and the square bracket), we get all the columns. When starting out in R, this is a bit counterintuitive. As a rule, we omit the column number to see all columns in a data frame. 
Similarly, if we leave out an index or range for the rows, we 
...
we'll discuss in a future lab. For now, the important thing is that we can carve up the data based on values of one or more variables.
...
```{r smokers-under-23, eval=TRUE}
under_23_and_smoke <- subset(cdc, cdc$smoke100 == TRUE & cdc$age >23)
head(under_23_and_smoke)
tail(under_23_and_smoke)
```
...
categorical data such as `smoke` and `gender` so now let's turn our attention to quantitative data.  Two common ways to visualize quantitative data are with box plots and histograms.  We can construct a box plot for a single variable with the following command.
...
4.  What does this box plot show? Pick another categorical variable from the 
    data set and see how it relates to BMI. List the variable you chose, 
    why you might think it would have a relationship to BMI, and indicate 
    what the figure seems to suggest. 
```{r boxplot-bmi-exer, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
boxplot(bmi ~ cdc$exerany)
```
    *The above boxplot demonstrates the rlationship between bmi and the exercise habits of the respondents.  It shows that the group who responded that the had exercised in the past month have a lower bmi than the group who did not exercise in the past month*
...
How do these two histograms compare? *These hisogram with 50 breaks give more detail about the exact ages of the respondents, where the defalt histogram groups ages*
...
```{r scatterplot-wt-deswt, eval=TRUE}
plot(cdc$weight, cdc$wtdesire, xlab="Current weight", ylab="Desired weight")
```
*This plot shows that most people are close to their desired weight*
...
str(wdiff)
```
*wdiff is an integer.  When wdiff is 0, there is no difference between the respondent current weight and the desired weight.  When wdiff is positive, the respondent current weight is less than than the desired weight.  When wdiff is negative, the respondent current weight is greater than the desired weight*

-   Describe the distribution of `wdiff` in terms of its center, shape, and 
    spread, including any plots you use. What does this tell us about how people feel about their current weight?
```{r dist-wdiff, eval=TRUE}
boxplot(wdiff)
summary(wdiff)
hist(wdiff, breaks = 50, ylab="Number of Respondents", xlab="Desired weight change in pounds")
```
  

-   Using numerical summaries and a side-by-side box plot, determine if men tend
    to view their weight differently than women.
```{r boxplot-wt-gender, eval=TRUE}
...
```{r sum-wt, eval=TRUE, echo= FALSE}
library(mosaic)
mean(cdc$weight)
sd(cdc$weight)
lzsc <- zscore(cdc$weight) >= -1 & zscore(cdc$weight) <= 1
table(lzsc)
barplot(table(lzsc))
table(lzsc)/20000
myz <- (cdc$weight - mean(cdc$weight)) / sd(cdc$weight)
myzl <- myz >= -1 & myz <= 1
table(myzl)
barplot(table(myzl))
table(myzl)/20000
```
    *The above steps:
    1.calculated mean & std eviation of the weight variable
    2.created a logical vector (lzsc) to indicate TRUE for every row that is within 1 std deviation of the mean, using the zscore function
    3.used the table function to display the contents of the lzsc vector
    4.used the barplot function to graphically display the contents of the lzsc vector
    5.divided the lzsc vector by the number of rows to get the proportions of the number of respondents within 1 std dev of the mean (71%)
    *check my work* 
    ***
    6.created a numeric vector (myz) to indicate the zscore for every element using the zscore formula.
    7,created a second logical vector (myzl) to indicate TRUE for every row that is whose weight is between -1 and 1 when subtractiing the sum of the mean and the std dev.  This should equal the number of people within 1 std dev of the mean using zscore
    7.divided the myzl by the number of rows to get the proportions of the number of respondents with a value of TRUE.(71%)
